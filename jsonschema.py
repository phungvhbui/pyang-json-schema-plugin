...
class JSONSchemaPlugin(plugin.PyangPlugin):
    ...
    def emit(self, ctx, modules, fd):
        ...
        description_str = "Generated by pyang from module %s" % root_stmt.arg
        result = {"title": root_stmt.arg,
        ...
        schema = produce_schema(root_stmt, main)
        result["properties"].update(schema)

        fd.write(json.dumps(result, indent=2))


def produce_schema(root_stmt, main_node):
    ...
    if hasattr(stmt, 'i_children'):
        for child in stmt.i_children:
            if child.keyword in producers:
                logging.debug("keyword hit on: %s %s",
                              child.keyword, child.arg)
                result[arg]["items"]["properties"].update(
...
def produce_choice(stmt):
    ...
    for child in stmt.substmts:
        logging.debug("checking on keywords with: %s %s",
                      child.keyword, child.arg)
        if child.keyword in ["container", "leaf", "list", "leaf-list"]:
            logging.debug("keyword hit on (short version): %s %s",
                          child.keyword, child.arg)
            result.update(producers[child.keyword](child))
...
def enumeration_trans(stmt):
    logging.debug("in enumeration_trans with stmt %s %s",
    ...
    logging.debug("In enumeration_trans for %s, returning %s",
                  stmt.arg, result)
    return result
...
def instance_identifier_trans(stmt):
    logging.debug("in instance_identifier_trans with stmt %s %s",
                  stmt.keyword, stmt.arg)
    result = {"type": "string"}
    return result
...
def produce_type(type_stmt):
    logging.debug("In produce_type with: %s %s",
...
def qualify_name(stmt):
    ...
    if stmt.parent.parent is None:  # We're on top
        ...
        logging.debug("In qualify_name with: %s %s on top",
                      stmt.keyword, stmt.arg)
        return pfx + ":" + stmt.arg
    if stmt.top.arg != stmt.parent.top.arg:  # Parent node is different
        pfx = stmt.top.arg
        logging.debug(
            "In qualify_name with: %s %s and parent is different", stmt.keyword, stmt.arg)
        return pfx + ":" + stmt.arg
    return stmt.arg
